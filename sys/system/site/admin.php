...
<?php
 ...
 if ($_GET) {
    ...
    // $row=pdo_select('SELECT aPic, is_data, Tb_index FROM maintable WHERE Tb_index=:Tb_index', $where);
    // unlink('../../img/'.$row['aPic']);
 ...
 function del_main($Tb_index)
 {
    $where=array('Tb_index'=>$Tb_index);
    $row=pdo_select('SELECT aPic, is_data, Tb_index, UseModuleID FROM maintable WHERE Tb_index=:Tb_index', $where);

    if ($row['is_data']=='0') {

      $pdo=pdo_conn();
      $sql=$pdo->prepare("SELECT Tb_index, UseModuleID, is_data FROM maintable WHERE parent_id=:parent_id");
      $sql->execute(['parent_id'=>$row['Tb_index']]);
      while ($row_d=$sql->fetch(PDO::FETCH_ASSOC)) {

        if ($row_d['is_data']=='0') {
          del_main($row_d['Tb_index']);
        }
        else{

          $row_mod=pdo_select("SELECT Tb_name FROM sysModule WHERE Tb_index=:Tb_index", ['Tb_index'=>$row_d['UseModuleID']]);
          pdo_delete($row_mod['Tb_name'], ['mt_id'=>$row_d['Tb_index']]);
      ...
      $row_mod=pdo_select("SELECT Tb_name FROM sysModule WHERE Tb_index=:Tb_index", ['Tb_index'=>$row['UseModuleID']]);
      pdo_delete($row_mod['Tb_name'], ['mt_id'=>$row['Tb_index']]);

      pdo_delete('maintable', ['Tb_index'=>$row['Tb_index']]);
    }
       ...
       $txt.='<td class="parent_td">
         ...
         $mode=pdo_select("SELECT Mod_name FROM sysModule WHERE Tb_index=:Tb_index", $where);
           ...
           if($row['not_delete']=='0'){
              ...
              $txt.='<a href="admin.php?Tb_index='.$row['Tb_index'].'" class="btn btn-white btn-sm" onclick="if (!confirm(\'確定要刪除 ['.$row['MT_Name'].'] ?\')) {return false;}">
