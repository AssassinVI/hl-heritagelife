...
if ($_GET) {
   ...
   if (!empty($_GET['Tb_index'])) {//移除熱門商品
    ...
    $param=array('HotArt'=>'0');

   	 pdo_update('appArticle',$param ,$where);
   }
   
   $pdo=pdo_conn();
   $sql=$pdo->prepare("SELECT Tb_index, aTitle, StartDate, OrderBy_hot, OnLineOrNot, aPic FROM appArticle WHERE HotArt='1' AND webLang=:webLang  ORDER BY OrderBy_hot DESC, StartDate DESC, Tb_index DESC");
...
<div class="wrapper wrapper-content animated fadeInRight">
	<div class="col-lg-12">
		...
		<p class="text-danger">首頁最多顯示12篇</p>
	   <div class="new_div">
					...
					<table class="table no-margin">
						<thead>
							<tr>
								...
								<th>代表圖</th>
								<th>標題</th>
								<th>排序 <span class="text-danger">(由大到小排序)</span></th>
								<th>日期</th>
								<th>目前狀態</th>
						...
						<?php $i=1; while ($row=$sql->fetch(PDO::FETCH_ASSOC)) {?>
							<tr>
								...
								<td><?php echo $online= $row['OnLineOrNot']==1 ? '上線中' : '<span class="text-danger">已下線</span>';?></td>
								...
								<a href="admin.php?MT_id=<?php echo $_GET['MT_id']?>&Tb_index=<?php echo $row['Tb_index'];?>" 
								   onclick="if (!confirm('確定要移除 [<?php echo $row['aTitle']?>] 變為一般文章?')) {return false;}">
