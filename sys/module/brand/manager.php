...
if ($_POST) {
    ...
    if (!empty($_POST['type']) && $_POST['type']=='delete') { //刪除
    	if (!empty($_POST['aPic'])) {
            ...
            pdo_update('appBrand', $param, $where);
        ...
        //----------------------- 多檔刪除 -------------------------------
    		...
    		$otr_file=pdo_select('SELECT OtherFile FROM appBrand WHERE Tb_index=:Tb_index', $sel_where);
	...
	if (empty($_POST['Tb_index'])) {//新增
		...
		$Tb_index='brand'.date('YmdHis').rand(0,99);
        $OnLineOrNot=empty($_POST['OnLineOrNot']) ? 0: 1 ;
        $hotBrand=empty($_POST['hotBrand']) ? 0: 1 ;
   ...
   //----------------------- 檔案判斷 -------------------------
	...
	$param=array(  'Tb_index'=>$Tb_index,
		       ...
		       'serial_brand'=>$_POST['serial_brand'],
		               'aPic'=>$aPic,
		          'OtherFile'=>$OtherFile,
		               'aTXT'=>$_POST['aTXT'],
     ...
     //----------------------- 檔案判斷 -------------------------
      if (!empty($_FILES['aPic']['name'])) {
      	...
      	if (test_img($_FILES['aPic']['name'])){
      		  ...
      		  pdo_update('appBrand', $aPic_param, $aPic_where);
      	}else{
      		location_up('admin.php?MT_id='.$_POST['mt_id'],'圖檔錯誤!請上傳圖片檔');
      		exit();
      	}
      ...
      $now_file =pdo_select("SELECT OtherFile FROM appBrand WHERE Tb_index=:Tb_index", $sel_where);
      	...
      	if (!empty($now_file['OtherFile'])) {
                ...
                pdo_update('appBrand', $OtherFile_param, $OtherFile_where);
...
if ($_GET) {
 	...
 	$row=pdo_select('SELECT * FROM appBrand WHERE Tb_index=:Tb_index', $where);
}
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					<form id="put_form" action="manager.php" method="POST" enctype='multipart/form-data' class="form-horizontal">
						...
						<div class="form-group">
							<label class="col-md-2 control-label" for="serial_type">流水號-品牌類型</label>
							<div class="col-md-4">
								<input type="text" class="form-control" id="serial_type" name="serial_type" maxlength="1" value="<?php echo $row['serial_type'];?>">
								<span class="text-danger">請輸入1個大寫英文</span>
							</div>
							<label class="col-md-2 control-label" for="serial_brand">流水號-品牌編碼</label>
							<div class="col-md-4">
								<input type="text" class="form-control" id="serial_brand" name="serial_brand" maxlength="3" value="<?php echo $row['serial_brand'];?>">
								<span class="text-danger">請輸入3個數字</span>
							</div>
						</div>
						...
						<div class="form-group">
							<label class="col-md-2 control-label" for="hotBrand">推薦品牌</label>
							<div class="col-md-10">
								<input style="width: 20px; height: 20px;" id="hotBrand" name="hotBrand" type="checkbox" value="1" <?php echo $check=$row['hotBrand']==1 ? 'checked' : ''; ?>  />
							</div>
						</div>
