...
if ($_POST) {
  ...
  for ($i=0; $i <count($_POST['OrderBy']) ; $i++) { 
    ...
    pdo_update('appBrand', $data, $where);
  }
}
...
if ($_GET) {
   ...
   if (!empty($_GET['Tb_index'])) {//刪除
    ...
    $where=array('Tb_index'=>$_GET['Tb_index']);
   	...
   	$del_row=pdo_select('SELECT aPic FROM appBrand WHERE Tb_index=:Tb_index', $where);
   	if (isset($del_row['aPic'])) { unlink('../../img/'.$del_row['aPic']); }

   	 pdo_delete('appBrand', $where);
   }
   
   $pdo=pdo_conn();
   $sql=$pdo->prepare("SELECT Tb_index, aTitle, aPic, OrderBy, hotBrand, OnLineOrNot FROM appBrand WHERE mt_id=:mt_id  ORDER BY OrderBy DESC, StartDate DESC, Tb_index DESC");
   $sql->execute(array( "mt_id"=>$_GET['MT_id']));
}
						...
						<?php 
						 $i=1;
						  while ($row=$sql->fetch(PDO::FETCH_ASSOC)) {
						  	$hotBrand=$row['hotBrand']==0 ? '':'<span class="label label-danger">推薦品牌</span>';
						?>
							<tr>
								<td><?php echo $i?></td>
								<td><img style="width: 100px;" src="../../img/<?php echo $row['aPic'] ?>" alt=""> </td>
