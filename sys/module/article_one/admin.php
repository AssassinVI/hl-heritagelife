...
if ($_GET) {
   ...
   $pdo=pdo_conn();
   $sql=$pdo->prepare("SELECT Tb_index, aTitle, StartDate, OrderBy, HotArt, OnLineOrNot, aPic FROM appArticle WHERE mt_id=:mt_id AND webLang=:webLang ORDER BY OrderBy DESC, StartDate DESC, Tb_index DESC");
			...
			<div class="panel-body">
				<div class="table-responsive">
					<table class="table no-margin">
						...
						<?php 
                         $row_url=pdo_select("SELECT use_web FROM maintable WHERE Tb_index=:Tb_index", ['Tb_index'=>$_GET['MT_id']]);
						$i=1; while ($row=$sql->fetch(PDO::FETCH_ASSOC)) {?>
							<tr>
								...
								<td><a href="../../../<?php echo $row_url['use_web'];?>?MT_id=<?php echo $_GET['MT_id'];?>" target="_blank">預覽網頁</a></td>
