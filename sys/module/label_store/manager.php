  ...
  // ======================== 刪除 ===========================
    ...
    if (!empty($_POST['type']) && $_POST['type']=='delete') { 
    	if (!empty($_POST['aPic'])) {
            ...
            pdo_update('appLabel', $param, $where);
            unlink('../../img/'.$_POST['aPic']);
    	}else{
        //----------------------- 多檔刪除 -------------------------------
    		$sel_where=['Tb_index'=>$_POST['Tb_index']];
    		$otr_file=pdo_select('SELECT OtherFile FROM appLabel WHERE Tb_index=:Tb_index', $sel_where);
	...
	location_up('admin.php?MT_id='.$_POST['mt_id'],'成功新增');
      ...
      if (!empty($_FILES['aPic']['name'])) {
        ...
        pdo_update('appLabel', $aPic_param, $aPic_where);
      ...
      if (!empty($_FILES['OtherFile']['name'][0])) {
      	...
      	$now_file =pdo_select("SELECT OtherFile FROM appLabel WHERE Tb_index=:Tb_index", $sel_where);
        ...
        pdo_update('appLabel', $OtherFile_param, $OtherFile_where);
      }
      	//--------------------------- END -----------------------------------
...
if ($_GET) {
 	...
 	$row=pdo_select('SELECT * FROM appLabel WHERE Tb_index=:Tb_index', $where);
}
?>
				...
				<div class="panel-body">
					<form id="put_form" action="manager.php" method="POST" enctype='multipart/form-data' class="form-horizontal">
						<div class="form-group">
							<label class="col-md-2 control-label" for="label_name">名稱</label>
							<div class="col-md-10">
								<input type="text" class="form-control" id="label_name" maxlength="15" name="label_name" value="<?php echo $row['label_name'];?>">
                <span class="text-danger">字數限制:15字</span>
							</div>
						</div>
