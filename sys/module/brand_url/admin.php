...
if ($_POST) {
  ...
  for ($i=0; $i <count($_POST['OrderBy']) ; $i++) { 
    ...
    pdo_update('appBrand_url', $data, $where);
  }
}
...
if ($_GET) {
   ...
   if (!empty($_GET['Tb_index'])) {//刪除
    ...
    $where=array('Tb_index'=>$_GET['Tb_index']);
   	...
   	$del_row=pdo_select('SELECT aPic FROM appBrand_url WHERE Tb_index=:Tb_index', $where);
   	if (isset($del_row['aPic'])) { unlink('../../img/'.$del_row['aPic']); }

   	 pdo_delete('appBrand_url', $where);
   }
   
   $pdo=pdo_conn();
   $sql=$pdo->prepare("SELECT Tb_index, aTitle, aPic, OrderBy, OnLineOrNot FROM appBrand_url WHERE mt_id=:mt_id  ORDER BY OrderBy DESC, StartDate DESC, Tb_index DESC");
