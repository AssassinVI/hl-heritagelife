...
if ($_POST) {
  ...
  for ($i=0; $i <count($_POST['OrderBy']) ; $i++) { 
    ...
    pdo_update('appLabel', $data, $where);
  }
}
...
if ($_GET) {
   ...
   if (!empty($_GET['Tb_index'])) {//刪除
    ...
    $where=['Tb_index'=>$_GET['Tb_index']];
   	 pdo_delete('appLabel', $where);
   }
   
   $pdo=pdo_conn();
   $sql=$pdo->prepare("SELECT Tb_index, label_name, OrderBy, OnLineOrNot FROM appLabel WHERE mt_id=:mt_id  ORDER BY OrderBy DESC, Tb_index DESC");
