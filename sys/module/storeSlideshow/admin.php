...
if ($_GET) {
   ...
   $pdo=pdo_conn();
   $sql=$pdo->prepare("SELECT Tb_index, aTitle, OrderBy, is_use, StartDate, EndDate FROM indexSlideshow WHERE mt_id=:mt_id AND webLang=:webLang ORDER BY OrderBy DESC, StartDate DESC, Tb_index DESC");
					...
					<table class="table no-margin">
						...
						<?php $i=1; while ($row=$sql->fetch(PDO::FETCH_ASSOC)) {?>
							<tr>
								...
								<td>
									<?php 
									   if ($row['StartDate'] <= date('Y-m-d H:i:s') && $row['EndDate'] >= date('Y-m-d H:i:s')) {
									   	 echo $online= $row['is_use']==1 ? '上線' : '已下線';
									   }
									   else{
									   	 echo $online= $row['is_use']==1 ? '活動結束' : '已下線';
									   }
									?>
								</td>
