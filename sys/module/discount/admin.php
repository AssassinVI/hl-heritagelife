...
if ($_GET) {
   ...
   $pdo=pdo_conn();
   $sql=$pdo->prepare("SELECT Tb_index, di_name, di_num, di_code, OrderBy, OnLineOrNot FROM appDiscount WHERE mt_id=:mt_id  ORDER BY OrderBy DESC, StartDate DESC");
					...
					<table class="table no-margin">
						...
						<?php $i=1; while ($row=$sql->fetch(PDO::FETCH_ASSOC)) {?>
							<tr>
								...
								<td><?php echo $row['di_num']; ?>折</td>
								<td><?php echo $row['di_code']; ?></td>
