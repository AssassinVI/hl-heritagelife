...
//--------------------------- 縮圖程式 PNG -------------------------
function ecstart_convert_png_NEW($src_file,$dst_file,$pic_width){
...
//--------------------------- 縮圖程式 -------------------------
function ecstart_convert_jpeg($src_file,$dst_file,$text_type,$text_image,$watermark_type,$watermark_image,$pic_width){
...
//-------------------------------- 數字補0 ---------------------------------------------
//-- $unit --
function number_zero($num, $unit=100)
{
  if ($num>=$unit) {
    return $num;
  }
  else{
    $txt_num='0'.number_zero($num, $unit/10);
    return $txt_num;
  }
}
  ...
  if(empty($Tb_index)){
      ...
      while ($row_nav=$sql->fetch(PDO::FETCH_ASSOC)) {
         ...
         if ($row_nav['is_data']=='0') {
           ...
           $icon=parent_icon($row_nav['Tb_index']);
            // -- 依實際HTML修改 --
            echo '
            <li class="nav-item dropdown">
              <a class="nav-link dropdown-toggle" href="javascript:void(0);" id="'.$row_nav['Tb_index'].'" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> <span style="background-image: url(sys/img/'.$icon.');"></span> '.$row_nav['MT_Name'].' </a>
               <div class="dropdown-menu" aria-labelledby="'.$row_nav['Tb_index'].'">';
         ...
         // -- 資料 --
            ...
            echo '
            <li class="nav-item">
              <a class="nav-link" href="'.$row_nav['use_web'].'?MT_id='.$row_nav['Tb_index'].'"><span style="background-image: url(sys/img/'.$icon.');"></span> '.$row_nav['MT_Name'].' </a>
            </li>';
         }
      } 
   }
   ...
   // ------------------------- 其他層 -------------------------------
      ...
      while ($row_nav=$sql->fetch(PDO::FETCH_ASSOC)){
         ...
         if ($row_nav['is_data']=='0'){
          // -- 依實際HTML修改 --
          echo '
              <a class="dropdown-item dropdown-toggle" href="javascript:void(0);" id="'.$row_nav['Tb_index'].'" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">'.$row_nav['MT_Name'].'</a>
               <div class="dropdown-menu" aria-labelledby="'.$row_nav['Tb_index'].'">';
         ...
         // -- 資料 --
           ...
           // -- 依實際HTML修改 --
           echo '<a class="dropdown-item" href="'.$row_nav['use_web'].'?MT_id='.$row_nav['Tb_index'].'">'.$row_nav['MT_Name'].'</a>';
         }
      }
   }
...
//--------------- 判斷ICON ----------------
function parent_icon($Tb_index)
 {
    $pdo=new PDO_fun;
    $icon=$pdo->select("SELECT icon,parent_id FROM maintable WHERE Tb_index=:Tb_index", ['Tb_index'=>$Tb_index]);
    if (empty($icon[0]['icon']) ) {
        
        $parent=$pdo->select("SELECT icon,parent_id FROM maintable WHERE Tb_index=:Tb_index", ['Tb_index'=>$icon[0]['parent_id']]);
        if (empty($parent[0]['parent_id']) && empty($parent[0]['icon'])) {
            return $parent[0]['icon'];
        }
        elseif (empty($parent[0]['icon'])) {
            parent_icon($parent[0]['parent_id']);
        }
        else{
          return $parent[0]['icon'];
        }
